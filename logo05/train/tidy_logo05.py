...
# Read the CSV file
df = pd.read_csv('logo_05_idx_60000_flt.csv')

# Delete columns
columns_to_delete = ['image_size', 'boxes_x', 'created_at', 'total_boxes', 
                     'det_polygons', 'asset_id_y', 'boxes_y' ,'box_type',
                     'mnt_uri', 'dbfs_uri', 'uri', 'det_scores', 'has_logo', 
                     'has_face'
                     ]
df = df.drop(columns=columns_to_delete)

# Rename columns
column_mapping = {'asset_id_x': 'asset_id', 's3_uri': 'uri'}
...
new_new_df.to_csv('logo_05_idx_60000_flt_cleared.csv', index=False)
new_new_df.to_parquet('logo_05_idx_60000_flt_cleared.parquet', index=False)